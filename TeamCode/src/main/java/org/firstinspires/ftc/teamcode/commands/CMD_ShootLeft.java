...
public class CMD_ShootLeft extends CommandBase {
    private final SUB_Shooter m_shooter;
    private final GamepadEx m_gamepad;
    private final RobotContainer m_robot;
    public CMD_ShootLeft(SUB_Shooter p_shooter, RobotContainer p_robot, GamepadEx p_gamepad){
        m_shooter = p_shooter;
        m_robot = p_robot;
        m_gamepad = p_gamepad;
    ...
    public void initialize(){
        m_robot.schedule(
            new SequentialCommandGroup(
                ...
                new InstantCommand(()-> m_shooter.setKickLeftPower(ShooterConstants.kKickOn))
            )
        );
    }
    ...
    public boolean isFinished(){
        return !m_gamepad.getButton(GamepadKeys.Button.LEFT_BUMPER);
    }
