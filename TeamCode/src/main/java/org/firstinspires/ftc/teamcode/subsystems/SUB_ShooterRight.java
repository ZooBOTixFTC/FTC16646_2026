...
@Config
public class SUB_ShooterRight extends SubsystemBase {
    ...
    public SUB_ShooterRight(OpMode p_opMode) {
        ...
        m_shooter = m_opMode.hardwareMap.get(DcMotorEx.class, "shooterMotorRight");

        m_shooter.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
        m_shooter.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
        m_shooter.setDirection(DcMotorSimple.Direction.REVERSE);
    }
    ...
    public void periodic() {
        ...
        packet.put("Right Vel", getVel());
        packet.put("Right Target Vel", Constants.ShooterConstants.kTuningMode ? vel : targetVel);
        packet.put("Right Volts", volts * m_voltageSensor.getVoltage());

        m_dashboard.sendTelemetryPacket(packet);

        m_opMode.telemetry.addData("Right Vel", getVel());
        m_opMode.telemetry.addData("Right Target Vel", getTargetVel());
        m_opMode.telemetry.addData("Right Volts", volts);
    }
}